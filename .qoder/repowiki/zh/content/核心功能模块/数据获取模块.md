...
**更新** 新增实时数据同步功能，支持当前价格获取和信号通知系统的集成。
...
**图示来源**
- [fetch_data.py:26-220](file://app/services/fetch_data.py#L26-L220)
...
**章节来源**
- [fetch_data.py:1-220](file://app/services/fetch_data.py#L1-L220)
...
## 架构总览
数据获取模块采用异步非阻塞请求，主数据源为雅虎财经（yfinance），备用数据源为Alpha Vantage（HTTP）。当主数据源异常或数据验证失败时，自动切换到备用数据源；若两者均失败，则抛出数据源错误异常。
...
- [fetch_data.py:86-160](file://app/services/fetch_data.py#L86-L160)
...
- _fetch_from_yahoo：使用yfinance同步获取历史数据，空数据时抛出异常
...
**更新** 新增实时数据同步功能，get_current_price方法提供当前价格获取能力。
...
**图示来源**
- [fetch_data.py:162-196](file://app/services/fetch_data.py#L162-L196)
...
- [fetch_data.py:108-160](file://app/services/fetch_data.py#L108-L160)
...
**图示来源**
- [models.py:19-134](file://app/database/models.py#L19-L134)
...
**图示来源**
- [analyze.py:30-168](file://app/api/analyze.py#L30-L168)
- [notification.py:35-100](file://app/services/notification.py#L35-L100)

**章节来源**
- [analyze.py:30-168](file://app/api/analyze.py#L30-L168)
- [notification.py:1-297](file://app/services/notification.py#L1-L297)
...
- 超时控制：统一的请求超时配置，防止长时间等待
- 实时同步：get_current_price方法提供快速当前价格获取，支持主备数据源回退

**更新** 新增实时数据同步性能优化，get_current_price方法采用5天日线数据获取，提高响应速度。
...
**更新** 新增通知系统故障排查指南，涵盖SMTP和Webhook配置问题。
...
- [fetch_data.py:108-160](file://app/services/fetch_data.py#L108-L160)
- [fetch_data.py:162-196](file://app/services/fetch_data.py#L162-L196)
- [config.py:38-42](file://app/core/config.py#L38-L42)
- [notification.py:94-99](file://app/services/notification.py#L94-L99)

## 结论
数据获取模块通过主备数据源的自动故障转移与严格的数据验证，构建了高可用的市场数据摄取能力。结合Pydantic模型与SQLAlchemy ORM，实现了从数据采集、清洗验证到结果持久化的完整闭环，为策略计算模块提供了稳定可靠的数据基础。新增的实时数据同步功能进一步增强了系统的响应能力和用户体验。

**更新** 新增实时数据同步和通知系统集成功能，显著提升了系统的实用性和自动化程度。

## 附录

### API调用示例（路径参考）
- 获取OHLCV历史数据：[fetch_ohlcv:44-84](file://app/services/fetch_data.py#L44-L84)
- 获取当前价格：[get_current_price:198-207](file://app/services/fetch_data.py#L198-L207)
- 备用数据源HTTP请求：[fetch_from_alpha_vantage:108-160](file://app/services/fetch_data.py#L108-L160)
- 实时数据同步接口：[get_market_data:213-219](file://app/services/fetch_data.py#L213-L219)
...
- 数据库模型：[models.py:19-134](file://app/database/models.py#L19-L134)

### 系统集成接口（路径参考）
- 分析API路由：[analyze.py:30-168](file://app/api/analyze.py#L30-L168)
...
- 通知服务：[notification.py:35-297](file://app/services/notification.py#L35-L297)
- 主应用入口：[main.py:17-120](file://app/main.py#L17-L120)