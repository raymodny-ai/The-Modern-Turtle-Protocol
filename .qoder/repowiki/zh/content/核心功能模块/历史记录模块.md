...
**更新** 新增了统计分析功能，支持按周期统计信号分布，增强了历史记录的分析能力。
...
**更新** 新增了统计分析服务，支持按周期统计信号分布和历史数据分析。
...
- 头寸计算：`account_equity`、`risk_amount`、`recommended_units`、`dollar_per_point`、`position_size`（Float）
...
主要功能：
- `save_analysis()`: 保存分析记录，处理各种参数和错误信息
...
**更新** 新增的统计分析功能进一步增强了历史记录模块的能力，支持更深入的数据洞察和业务分析。
